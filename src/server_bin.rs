...
async fn client_connection(
    ...
    tokio::task::spawn(client_rcv.forward(ws_sender).map(|result| {
        if let Err(err) = result {
            error!(
                "Failed to send message to client using websocket! Err: {}",
                err
            );
        }
    }));
...
async fn message_router(mut rcv: UnboundedReceiver<String>, clients: Clients) {
...
async fn main() {
    ...
    let (sender, rcv) = mpsc::unbounded_channel::<String>();
